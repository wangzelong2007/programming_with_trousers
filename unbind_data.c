...
int main(int argc, char **argv)
{
	...
	BYTE *rgbDataunBound;
	TSS_HKEY hUnBind_Key, hRecovered_UnBind_Key;
	//UINT32 ulDataLength;
	//FILE *fin, *fout;
	...
	result = Tspi_SetAttribData(hEncData,
					TSS_TSPATTRIB_ENCDATA_BLOB,
					TSS_TSPATTRIB_ENCDATABLOB_BLOB,
					encLen, encryptedData);
	DBG("Set Attrib data to hEncdata", result);
	result = Tspi_Context_GetKeyByUUID(hContext,
					TSS_PS_TYPE_SYSTEM,
					ESS_BIND_UUID,
					&hUnBind_Key);	
	DBG("Get unbinding key", result);
	result = Tspi_Key_LoadKey(hUnBind_Key,
				hSRK);
	...
	write(fileno(fout), rgbDataunBound, ulDataLength);
	fclose(fout);
